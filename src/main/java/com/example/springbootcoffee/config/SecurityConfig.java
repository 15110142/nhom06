package com.example.springbootcoffee.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().withUser("tuan").password(encoder.encode("thaituan11")).roles("ADMIN");
		auth.inMemoryAuthentication().withUser("employee").password(encoder.encode("123456")).roles("USER");
...
//		auth.inMemoryAuthentication().passwordEncoder(NoOpPasswordEncoder.getInstance()).withUser("sena").password("123456").roles("USER");
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().and().formLogin()//
				...
				.and().logout().logoutUrl("/j_spring_security_logout").logoutSuccessUrl("/?message=logout")
				.and()
